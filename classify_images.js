...
async function downloadFile(id, destPath) {
  ...
  console.log(`Downloaded ${id} to ${destPath} (${buffer.length} bytes)`);
}

async function classifyImage(filePath, id) {
  try {
  ...
  for (const f of files) {
    ...
    try {
      if (!fs.existsSync(destPath)) {
        await downloadFile(f.id, destPath);
      } else {
        console.log(`File already exists: ${destPath}`);
      }
      
      console.log(`Classifying ${f.name}...`);
      const classification = await classifyImage(destPath, f.id);
      if (classification) {
        console.log(`File ID ${f.id} (${f.name}) classified as:`, classification);
        mappings.push({
          id: f.id,
          name: f.name,
      ...
      console.error(`Failed process for ${f.name}:`, e);
    }
  }

  fs.writeFileSync('classified_mappings.json', JSON.stringify(mappings, null, 2));
  console.log('\nSaved all mappings to classified_mappings.json');
}
